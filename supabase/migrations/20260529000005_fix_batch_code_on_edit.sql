-- Fix batch code edits: rename inventory lots in place when only the batch changes,
-- and update batch_code when topping up an existing lot (case-only corrections).
...
declare
  ...
  v_old_batch text;
  v_new_batch text;
  v_only_batch boolean := false;
  v_rename_conflict boolean := false;
begin
  ...
  v_only_batch :=
    v_old_batch is distinct from v_new_batch
    and v_direction = v_old.direction
    and lower(trim(coalesce(v_payload ->> 'product_name', ''))) = lower(trim(v_old.product_name))
    and app.normalize_expiry(v_payload ->> 'expiry_date') is not distinct from v_old.expiry_date
  ...
  if v_only_batch then
    ...
    if not v_rename_conflict then
      ...
      set batch_code = v_new_batch, updated_at = now()
      where lower(il.product_name) = lower(trim(v_old.product_name))
        and lower(il.batch_code) = lower(v_old_batch)
        and coalesce(il.expiry_date, '1000-01-01'::date) = coalesce(v_old.expiry_date, '1000-01-01'::date)
      ...
      return jsonb_build_object('ok', true, 'updated', p_id, 'batch_renamed', true);
    end if;
  end if;
